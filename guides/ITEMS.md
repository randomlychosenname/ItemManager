...
This guide explains how a **custom item** is defined in ``items.yml``. All fields are **mandatory**, but **may be left empty**, so _you only need to set what you actually want to customize_.
...
Allows unsafe enchantment levels (0 to 255).
...
You may also stack multiple modifiers on multiple attributes for different equipment slots.
